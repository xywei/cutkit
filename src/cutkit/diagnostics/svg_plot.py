...
def render_loglog_chart_svg(chart: SvgLogLogChart) -> str:
    ...
    lines.append(
        f'<rect x="0" y="0" width="{chart.width}" height="{chart.height}" fill="#ffffff"/>'
    )

    for value in x_ticks:
        px = map_x(value)
        lines.append(
            f'<line x1="{px:.3f}" y1="{margin_top:.3f}" x2="{px:.3f}" y2="{margin_top + plot_height:.3f}" stroke="#ececec" stroke-width="1"/>'
        )
        lines.append(
            f'<text x="{px:.3f}" y="{margin_top + plot_height + 24:.3f}" text-anchor="middle" font-size="13" fill="#333">{escape(_format_tick(value))}</text>'
        )

    for value in y_ticks:
        py = map_y(value)
        lines.append(
            f'<line x1="{margin_left:.3f}" y1="{py:.3f}" x2="{margin_left + plot_width:.3f}" y2="{py:.3f}" stroke="#ececec" stroke-width="1"/>'
        )
        lines.append(
            f'<text x="{margin_left - 12:.3f}" y="{py + 4:.3f}" text-anchor="end" font-size="13" fill="#333">{escape(_format_tick(value))}</text>'
        )

    lines.append(
        f'<line x1="{margin_left:.3f}" y1="{margin_top:.3f}" x2="{margin_left:.3f}" y2="{margin_top + plot_height:.3f}" stroke="#222" stroke-width="1.2"/>'
    )
    lines.append(
        f'<line x1="{margin_left:.3f}" y1="{margin_top + plot_height:.3f}" x2="{margin_left + plot_width:.3f}" y2="{margin_top + plot_height:.3f}" stroke="#222" stroke-width="1.2"/>'
    )
    ...
    for row in chart.series:
        ...
        lines.append(
            f'<polyline points="{polyline_points}" fill="none" stroke="{escape(row.stroke)}" stroke-width="2.4"/>'
        )
        for x, y in pairs:
            lines.append(
                f'<circle cx="{map_x(x):.3f}" cy="{map_y(y):.3f}" r="3.2" fill="{escape(row.stroke)}"/>'
            )
    ...
    lines.append(
        f'<rect x="{legend_x:.3f}" y="{legend_y:.3f}" width="200" height="{legend_height:.3f}" fill="#ffffff" stroke="#d8d8d8"/>'
    )
    for index, row in enumerate(chart.series):
        ...
        lines.append(
            f'<text x="{legend_x + 40:.3f}" y="{y + 4:.3f}" font-size="13" fill="#222">{escape(row.label)}</text>'
        )

    lines.append(
        f'<text x="{chart.width / 2:.3f}" y="32" text-anchor="middle" font-size="18" fill="#111">{escape(chart.title)}</text>'
    )
    lines.append(
        f'<text x="{chart.width / 2:.3f}" y="{chart.height - 24:.3f}" text-anchor="middle" font-size="15" fill="#111">{escape(chart.x_label)}</text>'
    )
    lines.append(
        f'<text x="26" y="{chart.height / 2:.3f}" text-anchor="middle" transform="rotate(-90 26 {chart.height / 2:.3f})" font-size="15" fill="#111">{escape(chart.y_label)}</text>'
    )
